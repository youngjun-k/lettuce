package com.example.lettuce.api.user.dto.request;

import com.example.lettuce.domain.user.command.dto.UpdateFarmerProfileCommand;

import jakarta.validation.constraints.NotBlank;
import lombok.Getter;
import org.springframework.web.multipart.MultipartFile;
@Getter
public class UpdateFarmerProfileRequest extends UpdateUserRequest {

    @NotBlank(message = "농가 면허번호를 입력해주세요.")
    private String farmerLicenseNumber;

    @NotBlank(message = "농가 설명을 입력해주세요.")
    private String farmerDescription;

    public UpdateFarmerProfileCommand toCommand(MultipartFile profileImage) {
        return new UpdateFarmerProfileCommand(super.getNickname(), profileImage, farmerLicenseNumber,
                farmerDescription);
    }
}
