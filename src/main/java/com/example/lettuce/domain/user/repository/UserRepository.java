...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    // * When Spring Securiy is used, the user entity is detached from entity
    // manager.
    // * we need to fetch the user with profiles.
    // * The downfall of this is that every time the user is fetched with profiles,
    // even if the profile is not needed, the query is executed.
    ...
    Optional<User> findByEmailAndDeletedAtIsNullWithProfiles(@Param("email") String email);

    /*
     * Fetch Projection instead of Entity for performance
     * Fetch Projection is a feature that allows you to fetch a subset of the
     * entity's fields.
     * This can be more efficient than fetching the entire entity, especially if the
